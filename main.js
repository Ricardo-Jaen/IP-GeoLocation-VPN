...
modalSearchButton.addEventListener('click', () => {
    ...
    axios.get(`//54.242.128.133/php-intro-connection/getRecords.php?search=${encodeURIComponent(countryName)}`)
        .then(response => {
...
countryBtn.addEventListener('click', async () => {
    clearRecords();
    try {
        const ipResponse = await axios.get('//54.242.128.133/php-intro-connection/index.php');
        const countryCode3 = ipResponse.data.country_code3;
        if (countryCode3) {
            const response = await axios.get(`//54.242.128.133/php-intro-connection/getRecords.php?table=country&country_code3=${countryCode3}`);
...
cityBtn.addEventListener('click', () => {
    clearRecords();
    axios.get('//54.242.128.133/php-intro-connection/getRecords.php?table=city')
        .then(response => {
...
countryLanguageBtn.addEventListener('click', () => {
    clearRecords();
    axios.get('//54.242.128.133/php-intro-connection/getRecords.php?table=countrylanguage')
        .then(response => {
